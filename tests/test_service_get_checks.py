...
class ServiceTest(unittest.TestCase):
    ...
    def test_get_status_check_status(self):
        ...
        ret = self.imp.call('spv.services', 'get_checks', {'status': ['FINISHED', 'ERROR']})
        ...
        len_error = len(self.imp.call('spv.services', 'get_checks', {'status': 'ERROR'})['status'])
    ...
    def test_get_status_group_name(self):
        ...
        ret = self.imp.call('spv.services', 'get_checks', {'group_name': [list(groups.values())[0]['name']]})
    ...
    def test_get_status_object_address(self):
        ...
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'object_address': list(objects.values())[0]['address']})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_plugin_name(self):
        """ ['get_checks'] Get every status with on specific plugin name """
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'plugin_name': list(checks.values())[0]['plugin']})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_plugin_check(self):
        """ ['get_checks'] Get every status with on specific plugin check """
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'plugin_check': list(checks.values())[0]['plugin_check']})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_group_id(self):
        """ ['get_checks'] Get every status with on specific groups_id """
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0]})
        self._get_status_test_value(ret, groups, objects, checks)
    ...
    def test_get_status_check_id(self):
        ...
        ret = self.imp.call('spv.services', 'get_checks', {'check_id': list(checks.keys())[0]})
        self._get_status_test_value(ret, groups, objects, checks)
    ...
    def test_get_status_status_id(self):
        ...
        groups, objects, checks =  self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0]})
        ret_status = self.imp.call('spv.services', 'set_checks_status', [{'status_id': ret['status'][0]['status_id'],
            'sequence_id': 0, 'status': 'coucou !', 'message': 'salut', 'status_infos': {'key_status': 'value_status'}}])
        ret = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0], 'get_status_infos': True})
        self.assertEqual(list(ret['status'][0]['status_infos'].keys()), ['key_status'])
        self.assertEqual(ret['status'][0]['status_infos']['key_status']['key'], 'key_status')
        self.assertEqual(ret['status'][0]['status_infos']['key_status']['value'], 'value_status')
        self.assertEqual(ret['status'][0]['status_infos']['key_status']['status_id'], ret['status'][0]['status_id'])
    ...
    def test_get_status_limit(self):
        ...
        checks_second = self.imp.call('spv.services', 'create_checks',
                [{'plugin': '1', 'plugin_check': '2', 'name': '3', 'repeat': 100,
                    'repeat_on_error': 100 , 'group_id': list(groups.keys())[0]}])
        self.checks.append(list(checks_second.keys()))
        ret = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0], 'limit': 1})
    ...
    def test_get_status_object_info(self):
        ...
        groups, objects, checks =  self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0], 'get_object_infos': True})
    ...
    def test_get_status_check_info(self):
        ...
        groups, objects, checks =  self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0], 'get_check_infos': True})
    ...
    def test_get_status_detailed_infos(self):
        ...
        self.assertEqual(ret['checks'][list(ret['checks'].keys())[0]]['check_infos'], {'key_toto': 'value_toto'})
    ...
    def test_create_status(self):
        ...
        gets = {}
        gets[0] = self.imp.call('spv.services', 'get_checks', {'group_id': list(groups.keys())[0]})
        gets[1] = self.imp.call('spv.services', 'get_checks', {'check_id': list(checks.keys())[0]})
        gets[2] = self.imp.call('spv.services', 'get_checks', {'object_address': list(objects.values())[0]['address']})
