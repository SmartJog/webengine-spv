...
class ServiceTest(unittest.TestCase):
    ...
    def test_delete_groups(self):
        ...
        self.groups.append(list(groups.keys()))
        self.assertEqual(len(self.imp.call('spv.services', 'get_groups', {'group_id': list(groups.keys())[0]})), 1)
        self.imp.call('spv.services', 'delete_groups', list(groups.keys()))
        self.assertEqual(len(self.imp.call('spv.services', 'get_groups', {'group_id': list(groups.keys())[0]})), 0)
    ...
    def test_delete_checks(self):
        ...
        self.checks.append(list(checks.keys()))
        self.assertEqual(len(self.imp.call('spv.services', 'get_plugin_checks', {'check_id': list(checks.keys())[0]})), 1)
        self.imp.call('spv.services', 'delete_checks', list(checks.keys()))
        self.assertEqual(len(self.imp.call('spv.services', 'get_plugin_checks', {'check_id': list(checks.keys())[0]})), 0)

    def test_delete_objects(self):
        """ Create a objects, check and try to delete it """
        objects = self.imp.call('spv.services', 'create_objects', [{'address': 'toto'}])
        self.objects.append(list(objects.keys()))
        self.assertEqual(len(self.imp.call('spv.services', 'get_objects', {'obj_id': list(objects.keys())[0]})), 1)
        self.imp.call('spv.services', 'delete_objects', list(objects.keys()))
        self.assertEqual(len(self.imp.call('spv.services', 'get_objects', {'obj_id': list(objects.keys())[0]})), 0)
    ...
    def test_create_checks(self):
        ...
        # create
        checks = self.imp.call('spv.services', 'create_checks', [
            {'plugin': 'toto', 'plugin_check': 'test1', 'name': 'This is test 1',
             'repeat': 10, 'repeat_on_error': 10 },
            {'plugin': 'toto', 'plugin_check': 'test2', 'name': 'This is test 2',
             'repeat': 10, 'repeat_on_error': 10, 'infos': {
    ...
    def _get_test(self, params, data, api_func, test_func):
        ...
        for param, param_key in params:
            for key in list(data.keys()):
                ret = self.imp.call('spv.services', api_func, {param: data[key][param_key]})
        ...
        result_keys = [key for key in list(all_groups.keys()) if key in list(data.keys())]
        self.assertEqual(len(result_keys), len(list(data.keys())))

    def test_get_groups(self):
        """ Create groups and get it from spv with different filter """
        groups = self.imp.call('spv.services', 'create_groups', ['test_list_group1', 'test_list_group2', 'test_list_group3'])
        self.groups.append(list(groups.keys()))

        def test_result(ret, groups, key):
            self.assertEqual(ret[key]['id'], groups[key]['grp_id'])
            self.assertEqual(ret[key]['name'], groups[key]['name'])
        self._get_test([['group_id', 'grp_id'], ['group_name', 'name']], groups, 'get_groups', test_result)
    ...
    def test_get_objects(self):
        ...
        self._get_test([['obj_id', 'obj_id'], ['address', 'address'], ['creation_date', 'creation_date']],
                      objects, 'get_objects', test_result)
        ...
        ret = self.imp.call('spv.services', 'get_objects', {'creation_date': objects[keys[0]]['creation_date']})
    ...
    def test_get_checks(self):
        ...
        self._get_test([['chk_id', 'chk_id'], ['plugin', 'plugin'], ['plugin_check', 'plugin_check']],
                      checks, 'get_plugin_checks', test_result)

        keys = list(checks.keys())
        keys.sort()
        ret = self.imp.call('spv.services', 'get_plugin_checks', {'info_key': 'test_key_'})
        self.assertEqual(len(ret), 1)
        test_result(ret, checks, keys[1])

        ret = self.imp.call('spv.services', 'get_plugin_checks', {'info_value': 'test_val_'})
        self.assertEqual(len(ret), 1)
        test_result(ret, checks, keys[1])

        # check get multiple checks with on filter
        ret = self.imp.call('spv.services', 'get_plugin_checks', {'plugin_name': 'toto'})
    ...
    def test_update_groups(self):
        ...
        ret = self.imp.call('spv.services', 'update',
    ...
    def test_update_checks(self):
        ...
        checks = self.imp.call('spv.services', 'create_checks', [{'plugin': 'toto', 'plugin_check': 'test1',
                                                                  'name': 'test name', 'repeat': 10,
                                                                  'repeat_on_error': 10 }])
        self.checks.append(list(checks.keys()))
        id = list(checks.keys())[0]
        ret = self.imp.call('spv.services', 'update', {
            'checks':  {id: {'chk_id': id, 'name': 'salut', 'plugin': 'salut',
                                      'plugin_check': 'salut', 'repeat': 21, 'repeat_on_error': 2}}
            })
