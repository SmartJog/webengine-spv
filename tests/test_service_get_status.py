...
class ServiceTest(unittest.TestCase):
    ...
    def _get_status_tests_create_data(self):
        ...
        objects = self.imp.call('spv.services', 'create_objects',
                [{'address': 'test_address', 'infos': {'key_toto': 'value_toto'}, 'group_id': list(groups.keys())[0]}])
        ...
        checks = self.imp.call('spv.services', 'create_checks',
                [{'plugin': 'hey', 'plugin_check': 'salut', 'name': 'toi', 'repeat': 100,
                    'repeat_on_error': 100 , 'infos': {'key_toto': 'value_toto'}, 'group_id': list(groups.keys())[0]}])
    ...
    def _get_status_test_value(self, data, groups, objects, checks):
        ...
        self.assertTrue(data['objects'][list(objects.keys())[0]]['address'] == 'test_address')
        self.assertTrue(data['status'][list(data['status'].keys())[0]]['grp_id'] == list(groups.keys())[0])
        self.assertTrue(data['status'][list(data['status'].keys())[0]]['chk_id'] == list(checks.keys())[0])
        self.assertTrue(data['status'][list(data['status'].keys())[0]]['obj_id'] == list(objects.keys())[0])
    ...
    def test_get_status_group_name(self):
        ...
        ret = self.imp.call('spv.services', 'get_status', {'group_name': [list(groups.values())[0]['name']]})
    ...
    def test_get_status_object_address(self):
        ...
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'object_address': list(objects.values())[0]['address']})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_plugin_name(self):
        """ ['get_status'] Get every status with on specific plugin name """
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'plugin_name': list(checks.values())[0]['plugin']})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_plugin_check(self):
        """ ['get_status'] Get every status with on specific plugin check """
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'plugin_check': list(checks.values())[0]['plugin_check']})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_group_id(self):
        """ ['get_status'] Get every status with on specific groups_id """
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0]})
        self._get_status_test_value(ret, groups, objects, checks)
    ...
    def test_get_status_check_id(self):
        ...
        checks_second = self.imp.call('spv.services', 'create_checks',
                [{'plugin': '1', 'plugin_check': '2', 'name': '3', 'repeat': 100,
                    'repeat_on_error': 100 , 'group_id': list(groups.keys())[0]}])
        self.checks.append(list(checks_second.keys()))
        ret = self.imp.call('spv.services', 'get_status', {'check_id': list(checks.keys())[0]})
        self._get_status_test_value(ret, groups, objects, checks)

    def test_get_status_status_id(self):
        """ ['get_status'] Get status with a additional info on status """
        groups, objects, checks =  self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0], 'get_status_infos' : True})
        ret_status = self.imp.call('spv.services', 'set_checks_status', [{'status_id': ret['status'][list(ret['status'].keys())[0]]['status_id'],
            'sequence_id': 0, 'status': 'coucou !', 'message': 'salut', 'status_infos': {'key_status': 'value_status'}}])
        ret = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0], 'get_status_infos': True})
        self.assertEqual(list(ret['status'][list(ret['status'].keys())[0]]['status_infos'].keys()), ['key_status'])
        self.assertEqual(ret['status'][list(ret['status'].keys())[0]]['status_infos']['key_status']['value'], 'value_status')
        self.assertEqual(ret['status'][list(ret['status'].keys())[0]]['status_infos']['key_status']['key'], 'key_status')
        self.assertEqual(ret['status'][list(ret['status'].keys())[0]]['status_infos']['key_status']['status_id'], list(ret['status'].keys())[0])
    ...
    def test_get_status_limit(self):
        ...
        checks_second = self.imp.call('spv.services', 'create_checks',
                [{'plugin': '1', 'plugin_check': '2', 'name': '3', 'repeat': 100,
                    'repeat_on_error': 100 , 'group_id': list(groups.keys())[0]}])
        self.checks.append(list(checks_second.keys()))
        ret = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0], 'limit': 1})
    ...
    def test_get_status_object_info(self):
        ...
        groups, objects, checks =  self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0], 'get_object_infos': True})
    ...
    def test_get_status_check_info(self):
        ...
        groups, objects, checks =  self._get_status_tests_create_data()
        ret = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0], 'get_check_infos': True})
    ...
    def test_get_status_detailed_infos(self):
        ...
        self.assertEqual(list(ret['checks'][list(ret['checks'].keys())[0]]['check_infos'].keys()), ['key_toto'])
    ...
    def test_create_status(self):
        ...
        checks = self.imp.call('spv.services', 'create_checks',
            [{'plugin': 'test_plugin', 'plugin_check': 'check1', 'name': 'Name of test check',
              'repeat': 10, 'repeat_on_error': 10, 'group_id': list(groups.keys())[0]}])
        self.checks.append(list(checks.keys()))

        gets = {}
        gets[0] = self.imp.call('spv.services', 'get_status', {'group_id': list(groups.keys())[0]})
        gets[1] = self.imp.call('spv.services', 'get_status', {'check_id': list(checks.keys())[0]})
        gets[2] = self.imp.call('spv.services', 'get_status', {'object_address': list(objects.values())[0]['address']})
    ...
    def test_update_check(self):
        ...
        id1 = ret['status'][list(ret['status'].keys())[0]]['status_id']
        id2 = ret['status'][list(ret['status'].keys())[1]]['status_id']
        id3 = ret['status'][list(ret['status'].keys())[2]]['status_id']
