...
class ServiceTest(unittest.TestCase):
    ...
    def _get_status_test_value(self, data, groups, objects, checks):
        ...
        self.assertTrue(
            data["status"][list(data["status"].keys())[0]]["grp_id"]
            == list(groups.keys())[0]
        )
        self.assertTrue(
            data["status"][list(data["status"].keys())[0]]["chk_id"]
            == list(checks.keys())[0]
        )
        self.assertTrue(
            data["status"][list(data["status"].keys())[0]]["obj_id"]
            == list(objects.keys())[0]
        )
    ...
    def test_get_status_check_status(self):
        ...
        ret = self.imp.call(
            "spv.services", "get_status", {"status": ["FINISHED", "ERROR"]}
        )
        for status in ret["status"]:
            self.assertTrue(
                ret["status"][status]["check_status"] in ["FINISHED", "ERROR"]
            )
        ...
        len_error = len(
            self.imp.call("spv.services", "get_status", {"status": "ERROR"})["status"]
        )
    ...
    def test_get_status_group_id(self):
        ...
        groups, objects, checks = self._get_status_tests_create_data()
        ret = self.imp.call(
            "spv.services", "get_status", {"group_id": list(groups.keys())[0]}
        )
        self._get_status_test_value(ret, groups, objects, checks)
    ...
    def test_get_status_check_id(self):
        ...
        ret = self.imp.call(
            "spv.services", "get_status", {"check_id": list(checks.keys())[0]}
        )
        self._get_status_test_value(ret, groups, objects, checks)
    ...
    def test_get_status_status_id(self):
        ...
        ret_status = self.imp.call(
            ...
            [
                {
                    "status_id": ret["status"][list(ret["status"].keys())[0]][
                        "status_id"
                    ],
                    "sequence_id": 0,
                    "status": "coucou !",
        ...
        self.assertEqual(
            list(ret["status"][list(ret["status"].keys())[0]]["status_infos"].keys()),
            ["key_status"],
        )
        self.assertEqual(
            ret["status"][list(ret["status"].keys())[0]]["status_infos"]["key_status"][
                "value"
            ],
            "value_status",
        )
    ...
    def test_get_status_detailed_infos(self):
        ...
        self.assertEqual(
            list(ret["checks"][list(ret["checks"].keys())[0]]["check_infos"].keys()),
            ["key_toto"],
        )
